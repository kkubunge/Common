...
//------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------
int FOUP_SENSOR_CHECK(int *CommStatus)
{ // return 0 = None, 1 = Absent, 2 = Present
	...
	{
		*CommStatus = 1;
		if (Data1 == PRESENT && Data2 == PRESENT)
			return PRESENT2;
		else if (Data1 == ABSENT && Data2 == ABSENT)
			return ABSENT2;
		else
			return NONE2;
	}
			...
			if (FOUP_SENSOR_CHECK(&CommStatus) != Check_Value)
			{
				...
				if (FOUP_SENSOR_CHECK(&CommStatus) != Check_Value)
				{
					...
					while (TRUE)
					{
						...
						{														   // Get finally Alarm Recovery Status
		...
		for (i = 0; i < 2; i++)
		{
			...
			*Data = READ_DIGITAL(io, &CommStatus);
			if (!CommStatus)
			{
				...
				if (!WAIT_SECONDS((double)WAIT_TIME))
					return SYS_ABORTED;
			}
			else
			{
...
//------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------
Module_Status READ_DIGITAL_TRUE3(int io, int *Data)
{
	char pszFtnName[256] = "";
	int i, AlmResult, CommStatus;
	int count = 0;
	...
	while (TRUE)
	{
		...
		for (i = 0; i < 3; i++)
		{
			...
			if (!CommStatus)
			{
				...
				if (!WAIT_SECONDS((double)WAIT_TIME6))
					return SYS_ABORTED;
			}
			else
			{
...
void AGV_IO_DATA_SET(HWND hdlg, int iop, int io, int *old)
{
	...
	*old = i;
	if (i)
		KWIN_Item_String_Display(hdlg, io, "On");
	else
		KWIN_Item_String_Display(hdlg, io, "Off");
}
//--------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------
...
BOOL APIENTRY Console_Gui_For_EXCHANGE_Proc(HWND hdlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		if (!SIMULATION_LSWITCH_MODE)
			KWIN_Item_Hide(hdlg, ID_10_ON);
		if (!SIMULATION_USWITCH_MODE)
			KWIN_Item_Hide(hdlg, ID_11_ON);
	...
	case WM_COMMAND:
		...
		{
		case ID_1_ON:
			WRITE_DIGITAL(LP_AGV_INPUT_CS, ON, &CommStatus);
			return TRUE;
		case ID_1_OFF:
			WRITE_DIGITAL(LP_AGV_INPUT_CS, OFF, &CommStatus);
			return TRUE;
		case ID_2_ON:
			WRITE_DIGITAL(LP_AGV_INPUT_CS1, ON, &CommStatus);
			return TRUE;
		case ID_2_OFF:
			WRITE_DIGITAL(LP_AGV_INPUT_CS1, OFF, &CommStatus);
			return TRUE;
		case ID_4_ON:
			WRITE_DIGITAL(LP_AGV_INPUT_VALID, ON, &CommStatus);
			return TRUE;
		case ID_4_OFF:
			WRITE_DIGITAL(LP_AGV_INPUT_VALID, OFF, &CommStatus);
			return TRUE;
		case ID_5_ON:
			WRITE_DIGITAL(LP_AGV_INPUT_TRREQ, ON, &CommStatus);
			return TRUE;
		case ID_5_OFF:
			WRITE_DIGITAL(LP_AGV_INPUT_TRREQ, OFF, &CommStatus);
			return TRUE;
		case ID_6_ON:
			WRITE_DIGITAL(LP_AGV_INPUT_BUSY, ON, &CommStatus);
			return TRUE;
		case ID_6_OFF:
			WRITE_DIGITAL(LP_AGV_INPUT_BUSY, OFF, &CommStatus);
			return TRUE;
		case ID_7_ON:
			WRITE_DIGITAL(LP_AGV_INPUT_COMPLETE, ON, &CommStatus);
			return TRUE;
		case ID_7_OFF:
			WRITE_DIGITAL(LP_AGV_INPUT_COMPLETE, OFF, &CommStatus);
			return TRUE;
		case ID_8_ON:
			WRITE_DIGITAL(LP_AGV_INPUT_CONT, ON, &CommStatus);
			return TRUE;
		case ID_8_OFF:
			WRITE_DIGITAL(LP_AGV_INPUT_CONT, OFF, &CommStatus);
			return TRUE;
		case ID_9_ON:
			WRITE_DIGITAL(LP_FOUP_STS, ON, &CommStatus);
			return TRUE;
		case ID_9_OFF:
			WRITE_DIGITAL(LP_FOUP_STS, OFF, &CommStatus);
			return TRUE;
		case ID_10_ON:
...
Module_Status AGV_LOAD_CONTROL()
{
	...
	while (TRUE)
	{
		...
		{ // Complete Option
			...
			{ // SYS_SUCCESS, Foup Present, Complete Option
				return SYS_SUCCESS;
			}
		}
			...
			if (S_TIMER_CHECK(1, DEFAULT_TIME_OUT))
			{
				...
				if (AlmResult == ALM_IGNORE)
				{
					...
					if (Status != SYS_SUCCESS)
						...
						return SYS_SUCCESS; // SYS_SUCCESS, Foup Absent
...
Module_Status Load_Control()
{
	...
	{ // 1st while() : Carrier Load
			...
			if (Data == ON || Data2 == BUSY)
			{
				...
				else
				{
					...
					{ // wait foup present
						...
						if (READ_DIGITAL_TRUE(LP_FOUP_STS, &Data) == SYS_ABORTED)
							return SYS_ABORTED;
						if (Data == EXIST)
							break;
					}
		...
		{ // manual mode
			...
			if (READ_DIGITAL_TRUE3(LP_FOUP_STS, &Data) == SYS_ABORTED)
				return SYS_ABORTED;
	...
	{ // 2nd while() : Clamp Action
		...
		//... 2016.03.14
		if (READ_DIGITAL_TRUE(LP_FOUP_STS, &placeSensor) == SYS_ABORTED)
			return SYS_ABORTED;
		if (READ_DIGITAL_TRUE(PRESENT_STS, &presentSensor) == SYS_ABORTED)
			return SYS_ABORTED;
		...
		if (Data == AMHS)
		{
			Indicator(NONE);
			WRITE_DIGITAL_DOUBLE(LP_UNLOADING_SIGNAL, LP_LOADING_SIGNAL, NONE3, &CommStatus);
			if (WRITE_DIGITAL_TRUE(LP_CLAMP, CLAMP) == SYS_ABORTED)
				return SYS_ABORTED;
			if (SIMULATION_CASS_MODE == TRUE)
			{
				if (WRITE_DIGITAL_TRUE(LP_CLAMP_STS, CLAMP) == SYS_ABORTED)
					return SYS_ABORTED;
			}
			break;
		}
		else
		{ // Dynamic Load/Unload (Auto Clamp)
			Indicator(NONE);
			WRITE_DIGITAL_DOUBLE(LP_UNLOADING_SIGNAL, LP_LOADING_SIGNAL, NONE3, &CommStatus);
			if (WRITE_DIGITAL_TRUE(LP_CLAMP, CLAMP) == SYS_ABORTED)
				return SYS_ABORTED;
	...
	S_TIMER_READY();
	while (TRUE)
	{ // 3rd while() : Wait & Check Clamped Status
		_sleep(250);
	...
	if (RUN_FUNCTION_FREE(MID_CONTROL, "MID_READ") == SYS_ABORTED)
		return SYS_ABORTED;
...
Module_Status Unload_Control()
{
	...
	{ // 1st while() : Wait Unload
		...
		{ // manual mode
			...
			if (READ_DIGITAL_TRUE(LP_UNLOAD_BUTTON, &Data) == SYS_ABORTED)
				return SYS_ABORTED;
			if (Data == ON)
			{
				sprintf(StrBuffer, EVENT_STRING_TR_UNLOAD_RECV, CASSETTE_INDEX);
			...
			if (READ_DIGITAL(LP_CLAMP_STS, &CommStatus) != UNCLAMP)
			{
				...
				if (SIMULATION_CASS_MODE == TRUE)
				{
					if (WRITE_DIGITAL_TRUE(LP_CLAMP_STS, UNCLAMP) == SYS_ABORTED)
						return SYS_ABORTED;
				}
			}
	...
	S_TIMER_READY();
	while (TRUE)
	{ // 2rd while() : Wait & Check Unclamped Status
		_sleep(250);
	...
	{ // 3rd while() : wait foup unload
		...
		if (Data == AMHS)
		{
			...
			if (READ_DIGITAL_TRUE(LP_AGV_INPUT_CS, &Data) == SYS_ABORTED)
				return SYS_ABORTED;
			if (READ_DIGITAL_TRUE(LP_FOUP_STS, &Data3) == SYS_ABORTED)
				return SYS_ABORTED;
...
Module_Status Program_Main()
{
	...
	if (STRCMP_L(IdStr, "DISABLE"))
		id = 0;
	else if (STRCMP_L(IdStr, "ENABLE"))
		id = 1;
	else if (STRCMP_L(IdStr, "ENABLE2"))
		id = 2;
	else if (STRCMP_L(IdStr, "ENABLE3"))
		id = 3;
	else
	{
...
*/
	...
	{ // Manual
		...
		if (SIMULATION_AGV_MODE)
			WRITE_DIGITAL(LP_MODE_AGV_STS, MANUAL, &CommStatus);
		if (!CommStatus)
		{
			_sleep(1);
			WRITE_DIGITAL(LP_MODE_AGV_SET, MANUAL, &CommStatus);
	...
	{ // auto = amhs
		...
		if (SIMULATION_AGV_MODE)
			WRITE_DIGITAL(LP_MODE_AGV_STS, AMHS, &CommStatus);
		if (!CommStatus)
		{
			_sleep(1);
			WRITE_DIGITAL(LP_MODE_AGV_SET, AMHS, &CommStatus);
...
void Event_Message_Received()
{
	...
	if (READ_DIGITAL(LP_AMHS_MOVING, &CommStatus) != OFF)
		return;
