...
void		KGUI_Set_BACKGROUND(char *file)
{BackBmpLib = LoadLibrary(file); }
HINSTANCE	KGUI_Get_BACKGROUND()
{return BackBmpLib; }
//------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------
void Set_Prm_MINISCREEN_MODE(BOOL data)
{MINI_SCREEN_MODE = data; }
BOOL Get_Prm_MINISCREEN_MODE()
{return MINI_SCREEN_MODE; }
//--------------------------------------------------------------------------------------------------
void Set_Prm_TITLE_MODE(BOOL data)
{TITLE_MODE = data; }
...
//--------------------------------------------------------------------------------------------------
void Set_Prm_AUTO_DELETE_MONTH(int data)
{AUTO_DELETE_MONTH = data; }
int  Get_Prm_AUTO_DELETE_MONTH()
{return AUTO_DELETE_MONTH; }
//--------------------------------------------------------------------------------------------------
...
void Screen_Nav_Info(int no)
{
	...
	if ((no > 0) && (PROGRAM_GUI_SPAWNING == 0))
	{// 2003.07.12
...
void LView_Insert_Column(HWND hWnd, char *Msg, char *Msg2, char *Msg3, char *Msg4, char *Msg5, char *Msg6, char *Msg7, char *Msg8, char *Msg9, char *Msg10, char *Msg11, int id)
{
	LV_ITEM lvi;
...
void Operate_Data_Read(HWND hdlg)
{
	...
	do
	{
		...
		if (*Buffer != 0)
		{
			...
			// 11 / 10 12 : 38 : 57.538 [Pick(BM1)] A_PM3 Place<A1_Arm(0 $) : X_Axis[-0.26 / 0.26], Y_Axis[-0.04 / 0.04]><A2_Arm(0 $) : X_Axis[0.00 / 0.00], Y_Axis[0.00 / 0.00]><Limit[HI5 .00 / LOW0 .00]>

			// 11/10 15:42:04.864 SET_AWS_LIMIT
			z1 = 0;
			if (!STRFIND_SUBSTRING(Buffer, "SET_AWS_LIMIT"))
			{
				// Time ["11 / 10 12 : 38 : 57.538 "]
				z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, '[', time, z1, 255);
				if (z1 != z2)
				{
					// Not use string Dellete [" [Pick("]
					z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, '(', temp, z2, 255);
					z1 = z2;
					// Pick ["BM1"]
					z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, ')', pick, z2, 255);
					if (z1 != z2)
					{
						// Not use string Dellete [")] A_"]
						z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, '_', temp, z2, 255);
						z1 = z2;
						// Place ["PM3"]
						z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, ' ', place, z2, 255);
						if (z1 != z2)
						{
							// Not use string Dellete [" Place<"]
							z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, '<', temp, z2, 255);
							z1 = z2;
							// Arm ["A1"]
							z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, '_', arm_1, z2, 255);
							if (z1 != z2)
							{
								// Not use string Dellete ["_Arm(0 $) : X_Axis["]
								z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, '[', temp, z2, 255);
								z1 = z2;
								// X_Axis ["-0.26 / 0.26"]
								z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, ']', x_axis_1, z2, 255);
								if (z1 != z2)
								{
									// Not use string Dellete ["], Y_Axis["]
									z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, '[', temp, z2, 255);
									z1 = z2;
									// Y_Axis ["-0.04 / 0.04"]
									z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, ']', y_axis_1, z2, 255);
									if (z1 != z2)
									{
										// Not use string Dellete ["]><"]
										z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, '<', temp, z2, 255);
										// Arm ["A2"]
										z1 = z2;
										z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, '_', arm_2, z2, 255);
										if (z1 != z2)
										{
											// Not use string Dellete ["_Arm(0 $) : X_Axis["]
											z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, '[', temp, z2, 255);
											z1 = z2;
											// X_Axis ["0.00 / 0.00"]
											z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, ']', x_axis_2, z2, 255);
											if (z1 != z2)
											{
												// Not use string Dellete [", Y_Axis["]
												z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, '[', temp, z2, 255);
												z1 = z2;
												// Y_Axis ["0.00 / 0.00"]
												z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, ']', y_axis_2, z2, 255);
												if (z1 != z2)
												{
													// Not use string Dellete ["]><Limit["]
													z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, '[', temp, z2, 255);
													z1 = z2;
													// Limit ["HI5 .00 / LOW0 .00"]
													z2 = STR_SEPERATE_CHAR_WITH_POINTER(Buffer, ']', limit, z2, 255);
												}
											}
										}
									}
				...
				LView_Insert_Column(GetDlgItem(hdlg, IDC_DATA_LIST), no, time, pick, place, arm_1, x_axis_1, y_axis_1, arm_2, x_axis_2, y_axis_2, limit, i - 1);
	...
	_lclose(hFile);
	wsprintf(no, "%d item(s)", i);
	KWIN_Item_String_Display(hdlg, IDC_DATA_TOTAL, no);
...
void CompareView(HWND hdlg)
{
	...
	do {
		...
		if (*Buffer != 0)
		{
			...
			LView_Insert_Column(GetDlgItem(hdlg, IDC_DATA_LIST), Buffer1_2, Buffer1, Buffer2, Buffer3, LS, LE, RS, RE, Buffer8, LS2, LE2, i - 1);
...
//	int         szSize[4]   = { 55 ,    155,    140,  2000 };
	/*
	TCHAR       *szString[] = {"No", "Time", "Station", "Arm", "L_Start", "L_End", "R_Start", "R_End", "Arm", "L_Start", "L_End", "R_Start", "R_End", "Arm", "L_Start", "L_End", "R_Start", "R_End", "Arm", "L_Start", "L_End", "R_Start", "R_End"};
	...
	*/
	...
	TCHAR *szString[] = {"No", "Time", "Pick", "Place", "Arm", "X_Axis", "Y_Axis", "Arm", "X_Axis", "Y_Axis", "Limit"};
	...
	int szSize[23] = {35, 135, 55, 55, 55, 120, 120, 55, 120, 120, 120};
	int szSize_M[23] = {35, 135, 55, 55, 55, 120, 120, 55, 120, 120, 120};
	...
	switch (msg)
	{
		case WM_INITDIALOG:
			...
			KWIN_Item_Resize(hdlg, IDC_AWCANAL, Global_Pos_X[BUTTON_READ], Global_Pos_Y[BUTTON_READ], Global_Pos_XS[BUTTON_READ], Global_Pos_YS[BUTTON_READ]);										 // read again
...
*/
			if (Get_Prm_MINISCREEN_MODE())
				KWIN_LView_Init_Header(GetDlgItem(hdlg, IDC_DATA_LIST), LIST_HEADER_SIZE, szString, szSize_M);
			else
				KWIN_LView_Init_Header(GetDlgItem(hdlg, IDC_DATA_LIST), LIST_HEADER_SIZE, szString, szSize);
...
//						sprintf(SELECT_FILENAME, "%s/OP%s.log", HISTORY_DIRECTORY, Buffer); // 2004.01.07
			...
			switch (wParam)
			{
				...
				case IDC_AWCANAL :
					...
					GetWindowRect(hdlg, &analRect);
					//ClientToScreen(hdlg , &a);

					si.cb          = sizeof(si);
					si.dwFlags     = STARTF_USEPOSITION | STARTF_USESIZE|STARTF_USESHOWWINDOW;
					//si.dwX         = ScreenInfo.nX;
					//si.dwY         = ScreenInfo.nY;
					//si.dwXSize     = ScreenInfo.nWidth;
					//si.dwYSize     = ScreenInfo.nHeight;
					si.wShowWindow = SW_SHOW;

					sprintf(command, "%s %s %d %d %d %d", ".\\Function\\Function_Module_CTC\\OBJ_Gui_AWCScanLog\\AWCAnalysis.exe", "F:\\DataLog\\_Log", analRect.left, analRect.top, analRect.right, analRect.bottom );
					//BOOL state;
					CreateProcess(NULL, command, NULL,NULL, TRUE, NULL,NULL,NULL, &si, &pi);

					//ShellExecute(hdlg, "open", ".\\Function\\Function_Module_CTC\\OBJ_Gui_AWCScanLog\\AWCAnalysis.exe 1234", NULL, NULL, SW_SHOW);
					return TRUE;
				case IDRETRY :
...
//	REGIST_FILE_INFO("Gui Operate Log View 3.9(2006.05.29)"); // Reposition Part Append
	...
	//----------------------------------
	//----------------------------------
	while(PROGRAM_GUI_SPAWNING == 0)
	{_sleep(100); } // 2003.07.12
	//----------------------------------
	//----------------------------------
	//----------------------------------
	//----------------------------------
...
void Program_Enter_Code()
{
	...
	// 2006.02.16
	//============================================================================
	if (strcmp(HISTORY_DIRECTORY, "$") == 0)
	{// 2006.02.16
		_IO_GET_OPER_FOLDER(HISTORY_DIRECTORY, 128); // 2006.02.16
	}
	//==========================================================
	if (strlen(IDSTR_NAME) <= 0) strcpy(IDSTR_NAME, "TM_AWS_"); // 2004.01.07
	//
