...
char 	DRV_VERSION[50]	=	"FFU_DRV_Ver. 2.0";
char 	DRV_BUILDDATE[50] =	"2020.06.18";
// ]
...
BOOL ReadIOCValue(int ID1/*MCULID*/,int ID2/*LCUID*/,int ID3/*CHID*/,int *nMode,int *nSts,int *nRPM,int *nLowLimit,int *nHighLimit)
{
	...
	int  nETX         = 0;
	int  nRetry       = 0;
	int  RecvDataCnt  = 0; //Total Recv Data Count
...
void Digital_Out( int ID1 , int ID2 , int ID3 , int ID4 , int Data , int *Result ) {
	...
	switch(ID1) {
		...
		case 9998:
			g_nFFU_MAKER = Data; // 0 : SINSUNG , 1 : MARS
			break;
	}
